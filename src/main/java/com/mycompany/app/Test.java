...
public class Test {
	...
	static void benchGraalScriptEngine() throws IOException {
		...
		System.out.println("=== "+graaljsEngine.getClass().getName());
		benchScriptEngineIntl(graaljsEngine);
	}

	private static void benchScriptEngineIntl(ScriptEngine eng) throws IOException {
		String SOURCE = FileUtils.readFileToString(new File("D:\\workspace-sts\\graal-js-jdk11-maven-demo\\src\\main\\java\\src.js"));
		try {
			eng.eval(SOURCE);
		} catch (Exception ex) {
			ex.printStackTrace();
